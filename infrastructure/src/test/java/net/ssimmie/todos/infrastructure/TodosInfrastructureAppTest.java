...
class TodosInfrastructureAppTest {
  ...
  void createEnvironment_shouldCreateEnvironmentInstance() {
    ...
    // Environment values come from system properties or env vars, so we can't assert specific
    // values
  ...
  void constructor_shouldNotBeInstantiable() {
    ...
    assertThat(TodosInfrastructureApp.class.getDeclaredConstructors()[0].getParameterCount())
        .isZero();
  }
  ...
  void environmentBuilder_shouldHandleNullAccount() {
    ...
    Environment env = Environment.builder().account(null).region("eu-west-2").build();
  ...
  void environmentBuilder_shouldHandleNullRegion() {
    // Test that Environment.builder() can handle null values gracefully
    Environment env = Environment.builder().account("123456789012").region(null).build();
  ...
  void environmentBuilder_shouldHandleBothNull() {
    ...
    Environment env = Environment.builder().account(null).region(null).build();
