...
public class BenchmarkSimulation extends Simulation {
  ...
  HttpProtocolBuilder httpProtocol =
      http.warmUp("http://localhost:8181/actuator")
          .baseUrl("http://localhost:8181")
          .acceptHeader("application/json")
          .shareConnections();

  ScenarioBuilder healthCheckJourney =
      scenario("Todos Health Check").exec(HealthCheckAction.healthcheck());

  ScenarioBuilder coreJourney =
      scenario("Todos Core Journey")
          .exec(
...

  {
    setUp(
            ...
            coreJourney.injectOpen(
                constantUsersPerSec(30).during(Duration.ofMinutes(1)).randomized()))
